...
async function listRuleDirs(categoryDir: string): Promise<string[]> {
  ...
  return entries.filter((e) => e.isDirectory()).map((e) => path.join(categoryDir, e.name));
}

async function loadRule(ruleDir: string): Promise<{ id: string; rule: RuleDef } | null> {
  ...
  for (const catDir of await listCategoryDirs()) {
    for (const ruleDir of await listRuleDirs(catDir)) {
      ...
      try {
        ...
        throw new Error(
          `${ruleId}: fixtures dir missing — ${err instanceof Error ? err.message : String(err)}`
        );
      }
    }
  }
  ...
  for (const catDir of await listCategoryDirs()) {
    for (const ruleDir of await listRuleDirs(catDir)) {
      ...
      for (const f of await readdir(positiveDir)) {
        if (!f.endsWith(".yaml") && !f.endsWith(".cspec.yaml") && !f.endsWith(".json")) continue;
        const raw = await readFile(path.join(positiveDir, f), "utf-8");
        const doc = f.endsWith(".json") ? JSON.parse(raw) : yamlLoad(raw, { schema: JSON_SCHEMA });
        const r = await runRulesAgainstDocument({ rules: { [ruleId]: rule } }, doc, { source: f });
        assert.equal(
      ...
      for (const f of await readdir(negativeDir)) {
        ...
        const doc = f.endsWith(".json") ? JSON.parse(raw) : yamlLoad(raw, { schema: JSON_SCHEMA });
        const r = await runRulesAgainstDocument({ rules: { [ruleId]: rule } }, doc, { source: f });
        assert.ok(r.diagnostics.length > 0, `${ruleId}: negative fixture ${f} did NOT trigger`);
      }
    }
  }
});
